import {alignToGrid, containsRect, overlappingArea} from '../src/rect';
...
test('containsRect', () => {
  ...
  expect(containsRect(rect, {x: 0, y: 0, width: 2, height: 2})).toBe(true);
  expect(containsRect(rect, {x: 0, y: 0, width: 1, height: 1})).toBe(true);
  expect(containsRect(rect, {x: 1, y: 1, width: 1, height: 1})).toBe(true);

  expect(containsRect(rect, {x: 1, y: 0, width: 2, height: 2})).toBe(false);
  expect(containsRect(rect, {x: -1, y: 0, width: 2, height: 2})).toBe(false);
  expect(containsRect(rect, {x: 0, y: -1, width: 2, height: 2})).toBe(false);
  expect(containsRect(rect, {x: 0, y: 1, width: 2, height: 2})).toBe(false);
});
